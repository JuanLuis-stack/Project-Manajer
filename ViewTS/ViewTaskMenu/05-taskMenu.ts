...
export let tasksOptions: TasksOptions = {
    active: null
};

export function closeTasksOptions (): void{
    if(!tasksOptions.active)return;
...
export function createOptionsTask (callbacks: CreateOptionsTask){
    const {
        proyect,
        task,
        li,
    ...
    if(task.priority){priorityElement(priorityBtn)}
...
function editTask(callback: EditTask){
    const {
    task, 
    li, 
    p,
