...
let proyectActive: HTMLElement | null = null;

function closeProjectOptionsMenu(): void{
    if(!proyectActive)return;
...
function positionProjectOptionsMenu(button: HTMLButtonElement, menu: HTMLElement): void{
...
export function openProjectOptionsMenu(
...
export function createOptionsProyect (callbacks: CreateOptionsProyect | undefined): HTMLDivElement | undefined{
    if(!callbacks)return;
    ...
    if(callbacks.proyect.priority){priorityElement(priorityBtn)}
    ...
    if(!createTaskBtn || !taskAlmacen)return;
...
function proyectDOMRemove(){
    ...
    objContainer?.replaceChildren();
    taskAlmacen?.replaceChildren();
}

export function editProyect(proyect: Proyect, render: EditProyect, layoutSeleted: RenderlayoutSeleted){

    if(!render || !layoutSeleted)return;
    ...
    if(!titleContainer || !objContainer)return;
